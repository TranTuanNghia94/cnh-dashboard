...
function UpdateOrderPage() {
  ...
  const { mutateAsync, data } = useGetOrderByCode()
  ...
  useEffect(() => {
    if (data && data.data) {
      setDate(data.data.orderDate)
      setDateDelivery(data.data?.deliveryDate)
      setCustomerData(data.data?.customer)
      setAddressData(data.data?.customerAddress)
      setListLines(data.data?.orderLines)
    }
  }, [data])
  ...
  return (
    <div>
      <HeaderPageLayout idForm="updateOrderForm" title="Chỉnh sửa đơn hàng" />
      ...
      <div className="col-span-4 grid grid-cols-2 gap-x-4">
        <Card className="mt-4">
          ...
          <CardContent>
            <form id="formCreateOrder" className="grid grid-cols-5 gap-x-6">
              <div className="col-span-3">
                ...
                <Input
                  ...
                  defaultValue={data?.data?.contractNumber}
                />
              </div>
