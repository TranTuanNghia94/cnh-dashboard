...
export const Route = createLazyFileRoute('/_app/_wrapper/user/new')({
...
function NewUserPage() {
  ...
  const validateEmail = (email: string): boolean => {
    if (!email.includes(EMAIL_REGEX)) {
      toast({
        title: 'Thao tác thất bại',
        description:
          'Email không hợp lệ, vui lòng nhập email có đuôi @iesvietnam.com',
        variant: 'warning',
      })
      return false
    }
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const fields = ['lastName', 'firstName', 'email', 'role', 'password']
    ...
    if (
      !validatePassword(
        userData.password,
        formData.get('rePassword')?.toString() as string,
      )
    ) {
      return
    }
            ...
            >
              ...
              <div>
                <Label className="text-xs" htmlFor="password">
                  Mật khẩu mặt định <span className="text-red-600">*</span>
                </Label>
                <div className="flex">
                  <Input
