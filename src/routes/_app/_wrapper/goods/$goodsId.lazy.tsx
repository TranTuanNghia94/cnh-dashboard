...
import ProductTaxHistorySection from '@/components/product/product-tax-history-section'
...
export const Route = createLazyFileRoute('/_app/_wrapper/goods/$goodsId')({
...
function UpdateGoodsPage() {
  ...
  const productId = product?.data?.id ?? goodsId ?? ''
    ...
    <div>
      ...
      <Tabs defaultValue="general" className="mt-4">
        <TabsList className="w-full justify-start gap-2 rounded-xl bg-muted/40 p-1">
          <TabsTrigger value="general" className="gap-2 px-3 py-1.5 text-xs">
            <Package className="h-3.5 w-3.5" />
            Thông tin chung
          </TabsTrigger>
          <TabsTrigger value="tax-history" className="gap-2 px-3 py-1.5 text-xs" disabled={!productId}>
          ...
          <Card>
            <CardContent>
              <form
                id="updateGoodsForm"
                onSubmit={onSubmit}
                className="grid my-20 grid-cols-2 gap-x-20 gap-y-10"
              >
                <div className="grid grid-cols-2 gap-x-10 gap-y-5">
                    ...
                    >
                      ...
                      <SelectContent position="popper">
                        {categories?.data?.data &&
                          categories?.data?.data.map((val: ICategoryResponse) => (
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      defaultValue={product?.data?.code}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      defaultValue={product?.data?.unit1}
                    />
                  </div>
                  ...
                  <div className="col-span-2">
                    ...
                    <Input
                      ...
                      maxLength={200}
                      className="col-span-2"
                      defaultValue={product?.data?.misaCode}
                    />
                  </div>
                </div>
        ...
        <TabsContent value="tax-history" className="mt-4">
          {productId ? (
            <ProductTaxHistorySection productId={productId} currentTax={product?.data?.tax} />
          ) : null}
        </TabsContent>
      </Tabs>
    </div>
