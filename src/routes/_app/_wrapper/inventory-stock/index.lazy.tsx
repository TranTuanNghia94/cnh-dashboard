...
export const Route = createLazyFileRoute('/_app/_wrapper/inventory-stock/')({
...
function InventoryStockPage() {
  ...
  const queryAllTypes = async (
    req?: IPaginationAndSearch<IInventoryStockWhere>,
  ) => {
  ...
  return (
    <div>
      <DataTable
        listTools={null}
        fetchData={(req) =>
          queryAllTypes(req as IPaginationAndSearch<IInventoryStockWhere>)
        }
        total={data?.metadata?.total}
        title="DANH SÁCH TỒN KHO"
        data={
          data?.results?.map((item) => ({ ...item, refetch: queryAllTypes })) ||
          []
        }
